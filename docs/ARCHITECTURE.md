...
| Deployment | production-only release archive、container build、service composition、Nginx proxy、backup/migration/health-check/rollback | application business behavior；docs、CI metadata、browser evidence 不属于生产 release 输入 | `deploy/*`, `.github/workflows/deploy.yml` and root `docker-compose.yml` |
...
- `apps/web/src/app/i18n/index.tsx`: the application localization surface. It owns the active locale, translation lookup, `Intl` formatting, document language, and the pre-auth locale cache. Authenticated locale truth remains `user_settings.locale` through the Settings contract; Workspace applies that server preference after settings load, and feature owners consume only the i18n public surface.
...
`.github/workflows/ci.yml` owns CI verification only: an isolated MySQL 8 service starts with an empty `personal_workbench_test` database, Flyway applies and validates all versioned SQL migrations, then `npm run test:workflows:ci` executes the focused database suite and the Web workspace runs its feature behavior tests. The CI reporter fails on skipped/TODO/cancelled/failed database tests or a missing/empty summary. Typecheck, lint and build run only after the focused suites pass. `.github/workflows/deploy.yml` owns the GitHub trigger, tested commit selection, production-only release archive, and bounded SSH transfer; the archive excludes docs, CI metadata, and browser evidence. `deploy/release.sh` owns server-side backup, parallel image build, Flyway migration, Compose restart, health check, and failed-service rollback. Deployment does not own application behavior or database policy; Flyway remains the canonical migration executor.
...
### 2026-09-24 - Harden authenticated entry and production release path

- Context: The authenticated Today shell gated its Dashboard read on the explicit daily-carryover command. A transient carryover failure or an indefinitely stalled request could leave the first screen pending forever. Production deployment also transferred the entire repository, including browser evidence and documentation, before rebuilding services on the host.
- Decision: Daily carryover remains an explicit write owned by `daily-carryover.ts`, but the Web shell treats it as best-effort for initial read availability: failures are surfaced through global feedback, a bounded timeout aborts a stalled request, and the pure Dashboard query is still released. Production transfer owns a minimal release archive containing runtime source, migrations, package manifests, and deploy assets; documentation, CI metadata, scripts, and evidence are excluded. The deploy job uses bounded SSH keepalives/timeouts and `release.sh` builds API/Web images in parallel.
- Consequences: A carryover outage can show the current read snapshot without converting Dashboard GET into a write or deadlocking the route; a later explicit retry remains the mechanism for preparing carryover. Deployment no longer spends most of its time shipping non-runtime artifacts, while the server-side backup, Flyway migration, health check, and rollback order remain unchanged.
