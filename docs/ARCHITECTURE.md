...
>
> 状态：Baseline，2026-09-20。对架构事实或决策的有意变更必须同步更新本文档。
...
| Web Quick Notes feature | 随手记 capture/list/detail/edit/archive/recovery、转 Task 确认草稿、引用 Journal 的用户意图 | Task 发布事务、Journal server record、跨域 Search 真值 | `features/quick-notes/index.tsx` exports capture/list/detail surfaces；只经 Tasks 与 Writing 的公开能力连接 |
| Web Search / Settings features | Search 查询与 URL filters；个人资料、时区、类别启停、展示偏好和主动导出 UI | 各领域记录真值、奖励结算、历史日期重算、导出持久副本 | `features/search/index.tsx`、`features/settings/index.tsx`；AppShell 只组合路由与展示开关 |
...
| Quick Notes and controlled linking | `quick_notes` 原文、版本化生命周期、创建奖励事务；一次 Note 到一次 Task 的稳定来源关系 | Task lifecycle、Journal server record、Timeline 时间事实；Task DTO 不反向展开私人原文 | `/api/quick-notes/*`；`quick-note.ts`、`quick-note-linking.ts`；Web Quick Notes public surface |
...
- `apps/web/src/features/search/index.tsx` and `features/settings/index.tsx`: focused route surfaces for URL-backed domain search and user-controlled settings/export. They own their UI/query state but not the underlying domain records or reward policy.
