...
- Consequences: All Writing Slots are explicit opt-in for new accounts. Disabling a writing slot removes primary Today/navigation/quick-action entry points but does not delete or hide history, Search, or Export, and registration never rewrites an existing preference. Existing Categories and historical references remain intact; new accounts receive seven general-purpose Category defaults. A future `ProjectSection` may organize content only inside one Project and must not replace or duplicate TaskCategory. No Project Category table, generic preference platform, Growth editor, or second time/sleep truth is introduced.
...
| Projects、Habits、Finance、Integrations 按独立领域递增加入 | Projects owner、Task 当前单项目归属、occurrence 时间消费、Search/Export 已激活；其余领域仍保持隔离并按阶段加入 | R2A Projects 已激活；其余 B13+ 分域实施 |
