...
test("settings update future defaults and category disable preserves historical references", async () => {
  ...
  const updated = await api<{ profile: { displayName: string; timezone: string }; appearance: { reducedMotion: boolean }; rewards: { show: boolean } }>("/settings", { method: "PATCH", body: JSON.stringify({ displayName: "New Name", timezone: "Europe/London", reducedMotion: true, showRewards: false }) });
