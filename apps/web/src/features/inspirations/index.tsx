...
export function InspirationLibraryPage({ request, userId, selectedDate, projects = [], onError }: { request: Request; userId: number; selectedDate: string; projects?: InspirationProject[]; onError: (message: string, title?: string) => void }) {
  ...
  async function mutate(item: Inspiration, body: Record<string, unknown>, path = `/api/writing/inspirations/${item.id}`) { try { await request(path, { method: "PUT", body: JSON.stringify({ operationId: operationId(), expectedVersion: item.version, ...body }) }); await refresh(); } catch (error) { onError(apiError(error), "灵感没有更新"); } }
