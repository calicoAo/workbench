...
export function CalendarFeature({
  ...
  useEffect(() => {
    ...
    void request<{ taskIds: number[] }>(`/api/task-days?date=${currentState.draft.startDate}`).then((result) => { if (current) setRemoteCandidateIds(result.taskIds); }).catch((error) => { if (current) onError(errorMessage(error), tx("任务候选加载失败")); }).finally(() => { if (current) setCandidatesLoading(false); });
  ...
  async function createSchedule(event: FormEvent) {
    ...
    if (!taskId && !currentState.draft.title.trim()) {
      onError(tx("不关联任务时，需要写一下这段时间做了什么"));
      return;
    }
  ...
  function editSchedule(item: TimelineItem) {
    ...
    if (item.source === 1 || item.actualTimeClass === 2) { onError(item.source === 1 ? tx("计时记录只能通过原 Session 修正。") : tx("历史实际记录为只读。"), tx("此记录不可直接编辑")); return; }
      ...
      <section className="glass-panel p-3 xl:h-full">
        <div className="panel-header">
          <div className="flex items-center gap-2">
            ...
            <h2 className="section-title">{tx("小时记录")}</h2>
          </div>
          <Button variant="primary" size="sm" type="button" onClick={() => setState((current) => ({ ...current, editorOpen: true, editingId: null, overlapConflict: false }))}>
            <Plus size={14} />{tx("记录")}</Button>
        </div>

        <div className="mb-2 grid grid-cols-2 gap-1.5">
          <Metric label={tx("已记录")} value={actualSeconds === undefined ? formatDuration(actualMinutes) : formatSeconds(actualSeconds)} />
          <Metric label={tx("安排")} value={tx("{value0}段", { value0: plannedCount })} />
        </div>
        <div className="mb-2 flex items-center gap-3 px-1 text-[10px] text-soft">
          <span className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-mint-500" />{tx("实际")}</span>
          <span className="inline-flex items-center gap-1"><i className="h-2 w-2 rounded-full border border-dashed border-pink-300 bg-white/80" />{tx("安排")}</span>
        </div>
...
function TimeBlockDialog({ categories, tasks, projects, draft, editing, overlapConflict, candidatesLoading, onChange, onClose, onSubmit, onKeepExcluded }: {
  ...
  return createPortal(
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <div ref={shellRef} className="modal-shell" role="dialog" aria-modal="true" aria-label={editing ? tx("修正时间块") : tx("记录时间块")} onMouseDown={(event) => event.stopPropagation()}>
        <form className="time-modal" onSubmit={onSubmit}>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-semibold">{editing ? tx("修正时间块") : tx("记录时间块")}</h3>
            <IconButton size="sm" label={tx("关闭")} type="button" onClick={onClose}><X size={15} /></IconButton>
          </div>

          <div className="time-editor-primary-grid">
            <select aria-label={tx("关联任务")} className="field" value={draft.taskId} onChange={(event) => { const task = tasks.find((item) => String(item.id) === event.target.value); onChange({ taskId: event.target.value, projectId: task?.projectId ? String(task.projectId) : "" }); }}>
              <option value="">{candidatesLoading ? tx("正在读取当日接取任务...") : draft.kind === "1" ? tx("不关联任务") : tx("不关联任务")}</option>
              {tasks.map((task) => <option key={task.id} value={task.id}>{task.title}</option>)}
            </select>
            <select aria-label={tx("记录类型")} className="field" value={draft.kind} onChange={(event) => onChange({ kind: event.target.value })}>
              <option value="1">{tx("实际")}</option>
              <option value="0">{tx("安排")}</option>
            </select>
          </div>

          {draft.kind === "1" ? <label className="task-form-field mt-2"><span>{tx("发生时项目")}</span><select aria-label={tx("发生时项目")} className="field" value={draft.projectId} onChange={(event) => onChange({ projectId: event.target.value })}><option value="">{tx("Inbox / 无项目")}</option>{projects.filter((project) => !project.archivedAt && project.status !== 3).map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label> : null}

          {!draft.taskId && (
            <div className="mt-2 grid grid-cols-[1fr_128px] gap-2">
              <input aria-label={tx("时间块标题")} className="field" placeholder={tx("这段时间做了什么")} value={draft.title} onChange={(event) => onChange({ title: event.target.value })} />
              <select aria-label={tx("时间块分类")} className="field" value={draft.categoryId} onChange={(event) => onChange({ categoryId: event.target.value })}>
          ...
          <div className="time-editor-date-grid">
            <label className="text-[11px] text-soft">{tx("开始日期")}<input aria-label={tx("开始日期")} className="field mt-1" type="date" value={draft.startDate} onChange={(event) => onChange({ startDate: event.target.value, ...(draft.kind === "0" ? { endDate: event.target.value } : {}) })} /></label>
            <label className="text-[11px] text-soft">{tx("开始时间")}<input aria-label={tx("开始时间")} className="field mt-1" type="time" value={draft.start} onChange={(event) => onChange({ start: event.target.value })} /></label>
            <label className="text-[11px] text-soft">{tx("结束日期")}<input aria-label={tx("结束日期")} className="field mt-1" type="date" value={draft.endDate} disabled={draft.kind === "0"} onChange={(event) => onChange({ endDate: event.target.value })} /></label>
            <label className="text-[11px] text-soft">{tx("结束时间")}<input aria-label={tx("结束时间")} className="field mt-1" type="time" value={draft.end} onChange={(event) => onChange({ end: event.target.value })} /></label>
          </div>
          <input aria-label={tx("时间块备注")} className="field mt-2" placeholder={tx("备注，可不填")} value={draft.note} onChange={(event) => onChange({ note: event.target.value })} />
          {draft.kind === "1" && draft.taskId ? <label className="manual-complete-option"><input type="checkbox" checked={draft.completeTask} onChange={(event) => onChange({ completeTask: event.target.checked })} />{tx("同时完成悬赏（同一事务）")}</label> : null}
          {draft.kind === "1" ? <p className="manual-actual-note">{tx("补录投入 ≠ 完成 Task；默认只记录实际投入。")}</p> : null}
          {overlapConflict ? <div className="overlap-conflict" role="alert"><strong>{tx("与已有投入重叠")}</strong><span>{tx("调整开始/结束时间，或保留为不计入汇总的附注。")}</span><button type="button" onClick={onKeepExcluded}>{tx("保留附注，不计入投入汇总")}</button></div> : null}

          <div className="mt-4 flex justify-end gap-2">
            <Button variant="secondary" type="button" onClick={onClose}>{tx("取消")}</Button>
            <Button variant="primary" type="submit">{tx("保存")}</Button>
          </div>
        </form>
      </div>
    </div>,
...
function CategoryOptions({ categories }: { categories: Category[] }) {
  ...
  return <>{DIMENSIONS.map((dimension) => {
    ...
    return options.length ? <optgroup key={dimension.key} label={tx(dimension.label)}>{options.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</optgroup> : null;
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className="metric-card"><p className="text-[11px] text-soft">{tx(label)}</p><p className="mt-0.5 text-base font-semibold">{value}</p></div>;
}
