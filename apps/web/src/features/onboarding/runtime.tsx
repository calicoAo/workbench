...
function OnboardingRuntime({ children, request, userId, tasks, assignments, currentSession, hasActualTime, onError }: { children: ReactNode; request: Request; userId: number; tasks: TaskSnapshot[]; assignments: number[]; currentSession: CurrentSession | null; hasActualTime: boolean; onError: (message: string, title?: string) => void }) {
  ...
  async function advance(stepId: TutorialStep["id"]) {
    ...
    } catch (error) { onError(error instanceof Error ? error.message : "新手教学状态没有保存", "教学状态异常"); }
  }
  async function start() {
    try { const next = await request<Progress>(`/api/onboarding/flows/${coreLoopFlow.id}/start`, { method: "POST" }); tutorialStarted.current = true; setProgress(next); initialTaskIds.current = new Set(tasks.map((task) => task.id)); await queryClient.invalidateQueries({ queryKey: ["onboarding", userId] }); } catch (error) { onError(error instanceof Error ? error.message : "新手教学无法开始", "新手教学异常"); }
  }
  async function skip() { try { const next = await request<Progress>(`/api/onboarding/flows/${coreLoopFlow.id}/skip`, { method: "POST" }); setProgress(next); await queryClient.invalidateQueries({ queryKey: ["onboarding", userId] }); } catch (error) { onError(error instanceof Error ? error.message : "新手教学无法跳过", "新手教学异常"); } }
...
function WelcomeDialog({ onStart, onSkip }: { onStart: () => void; onSkip: () => void }) {
  ...
  return <div className="onboarding-backdrop" role="presentation"><section ref={ref} className="onboarding-welcome" role="dialog" aria-modal="true" aria-labelledby="onboarding-welcome-title"><p className="route-eyebrow">Personal Workbench</p><h1 id="onboarding-welcome-title">欢迎来到 Workbench</h1><p>这里不是普通的待办清单。<br />把想做的事发布成悬赏，接取它、真正开始行动，完成后的时间和行动会逐渐变成自己的成长记录。</p><div className="onboarding-actions"><button type="button" className="onboarding-secondary" onClick={onSkip}>直接进入</button><button type="button" className="onboarding-primary" onClick={onStart}>开始新手教学</button></div></section></div>;
}
