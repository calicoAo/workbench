...
function WelcomeDialog({ onStart, onSkip }: { onStart: () => void; onSkip: () => void }) {
  ...
  return <div className="onboarding-backdrop" role="presentation"><section ref={ref} className="onboarding-welcome" role="dialog" aria-modal="true" aria-labelledby="onboarding-welcome-title"><p className="route-eyebrow">✨ 养成系统</p><h1 id="onboarding-welcome-title">欢迎来到养成系统</h1><p>这里不是普通的待办清单。<br />把想做的事发布成悬赏，接取它、真正开始行动，完成后的时间和行动会逐渐变成自己的成长记录。</p><div className="onboarding-actions"><button type="button" className="onboarding-secondary" onClick={onSkip}>直接进入</button><button type="button" className="onboarding-primary" onClick={onStart}>开始新手教学</button></div></section></div>;
}
