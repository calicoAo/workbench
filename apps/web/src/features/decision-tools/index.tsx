...
export function DecisionToolsFeature({
  ...
  async function loadRecords() {
    try {
      ...
      onError(error instanceof Error ? error.message : "小工具记录加载失败", "小工具记录加载失败");
    }
  }
        ...
        <div className="tool-modal-grid">
          <div className="space-y-2">
            ...
            <div className="grid gap-2 md:grid-cols-2">
              <label className="space-y-1"><span className="tool-label">好处</span><textarea className="journal-input min-h-36" placeholder="会带来什么收益、成长、轻松感？" value={props.benefits} onChange={(event) => props.onBenefitsChange(event.target.value)} /></label>
              <label className="space-y-1"><span className="tool-label">坏处</span><textarea className="journal-input min-h-36" placeholder="会付出什么成本、风险、精力？" value={props.drawbacks} onChange={(event) => props.onDrawbacksChange(event.target.value)} /></label>
            </div>
            <div className="grid gap-2 md:grid-cols-[1fr_1fr_120px]">
              <label className="space-y-1"><span className="tool-label">好处权重</span><select className="field" value={props.benefitScore} onChange={(event) => props.onBenefitScoreChange(event.target.value)}><ScoreSelectOptions /></select></label>
              <label className="space-y-1"><span className="tool-label">坏处权重</span><select className="field" value={props.drawbackScore} onChange={(event) => props.onDrawbackScoreChange(event.target.value)}><ScoreSelectOptions /></select></label>
              <div className={`decision-score ${balance >= 0 ? "decision-score-positive" : "decision-score-negative"}`}><span>倾向</span><strong>{balance > 0 ? `+${balance}` : balance}</strong></div>
            </div>
            ...
            <div className="flex justify-end gap-2"><button className="icon-button w-auto px-4" type="button" aria-label="取消" onClick={props.onClose}>取消</button><button className="primary-button px-5" type="submit">保存决策</button></div>
          </div>
          <ToolRecordList emptyText="保存后会在这里看到决策记录。" items={props.records.map((item) => ({ id: item.id, title: item.theme, meta: `${item.decisionDate} · 好处 ${item.benefitScore} / 坏处 ${item.drawbackScore}`, body: item.conclusion || item.benefits }))} />
        </div>
      </form>
    </FeatureModal>
  );
  ...
  return (
    <FeatureModal onClose={props.onClose}>
      <form className="tool-modal" onSubmit={props.onSubmit}>
        ...
        <div className="tool-modal-grid">
          <div className="space-y-2">
            <label className="space-y-1"><span className="tool-label">想达成的效果</span><textarea className="journal-input min-h-28" placeholder="比如：我想稳定开始画画，不再只停留在想法里。" value={props.desiredEffect} onChange={(event) => props.onDesiredEffectChange(event.target.value)} /></label>
            <label className="space-y-1"><span className="tool-label">现在的阻力</span><textarea className="journal-input min-h-28" placeholder="比如：一想到要开始就觉得麻烦，怕画得不好。" value={props.resistance} onChange={(event) => props.onResistanceChange(event.target.value)} /></label>
            <button className="primary-button w-full px-5" type="submit" disabled={props.loading}>{props.loading ? "生成中..." : "生成心理桥梁"}</button>
            {latest && <article className="bridge-result"><p className="whitespace-pre-wrap text-[12px] leading-6 text-ink">{latest.bridgeText}</p>{latest.nextStep && <p className="mt-2 text-[12px] font-semibold text-mint-700">下一步：{latest.nextStep}</p>}{latest.reassurance && <p className="mt-2 text-[12px] text-soft">{latest.reassurance}</p>}</article>}
          </div>
          <ToolRecordList emptyText="生成后会在这里看到心理桥梁记录。" items={props.records.map((item) => ({ id: item.id, title: item.desiredEffect, meta: item.bridgeDate, body: item.nextStep || item.bridgeText }))} />
        </div>
      </form>
    </FeatureModal>
  );
