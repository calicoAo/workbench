...
export function WaterFeature({
  ...
  return <section className="glass-panel p-3">
      ...
      <div className="water-progress-wrap">
        <div className={`water-progress ${plan.isDue ? "is-due" : ""}`} aria-label={tx("饮水进度 {value0} 杯", { value0: water.cups })}>
          {Array.from({
          length: 8
        }, (_, index) => {
...
export function waterTimelineItems(water: WaterRecord | null): WaterTimelineItem[] {
  if (!water) return [];
  return parseDrinkTimes(water).slice(0, water.cups).map((drinkTime, index) => {
