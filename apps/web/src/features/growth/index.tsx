...
function InvestmentCharts({ dimensions }: { dimensions: GrowthOverview["dimensions"] }) {
  ...
  return <section className="growth-charts"><div className="growth-chart-panel"><div className="growth-section-title"><h2>成长投入占比</h2></div><svg className="growth-donut" role="img" aria-label="成长投入占比" viewBox="0 0 110 110">{items.map((item) => { const length = item.share * circumference; const segment = <circle key={item.id} cx="55" cy="55" r="42" fill="none" stroke={item.color} strokeWidth="14" strokeDasharray={`${length} ${circumference - length}`} strokeDashoffset={-offset} transform="rotate(-90 55 55)" />; offset += length; return segment; })}<text x="55" y="53" textAnchor="middle" className="fill-ink text-[10px] font-bold">投入</text><text x="55" y="66" textAnchor="middle" className="fill-soft text-[8px]">占比</text></svg><div className="growth-chart-legend">{items.map((item) => <span key={item.id}><i style={{ backgroundColor: item.color }} />{item.name} {Math.round(item.share * 100)}%</span>)}</div></div><div className="growth-chart-panel"><div className="growth-section-title"><h2>成长投入</h2></div><svg className="growth-radar" role="img" aria-label="成长投入雷达" viewBox="0 0 200 200"><polygon points="100,20 180,100 100,180 20,100" fill="none" stroke="rgba(38,50,56,.14)" />{items.length > 1 ? <polygon points={radarPoints} fill="rgba(53,201,154,.2)" stroke="#35C99A" strokeWidth="2" /> : null}</svg><p className="growth-chart-note">只展示期间投入分布。</p></div></section>;
}
