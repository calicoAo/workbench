...
describe("Hero Growth V1", () => {
  ...
  it("shows a clear zero-data empty state without NaN", async () => { const emptyRequest = vi.fn(async (path: string) => path === "/api/growth/dimensions" ? config : { ...overview, summary: { actualMinutes: 0, mappedActualMinutes: 0, completedTaskCount: 0, habitCompletedCount: 0 }, dimensions: overview.dimensions.map((item) => ({ ...item, actualMinutes: 0, share: 0, completedTaskCount: 0, lastActivityDate: null })), unmapped: { actualMinutes: 0, completedTaskCount: 0, lastActivityDate: null }, recent: [] }) as Request; render(wrapper(<GrowthPage request={emptyRequest} userId={1} date="2026-09-24" onError={vi.fn()} />)); expect(await screen.findByText("还没有成长记录")).toBeTruthy(); expect(document.body.textContent).not.toContain("NaN"); });
});
