...
export function QuickNoteDetailPage({ request, userId, noteId, selectedDate, recordTimezone = "Asia/Shanghai", categories = [], projects = [], onQuoteToJournal = () => ({ status: "inserted" }) }: { request: Request; userId: number; noteId: number; selectedDate: string; recordTimezone?: string; categories?: CategoryOption[]; projects?: ProjectOption[]; onQuoteToJournal?: (input: { noteId: number; targetDate: string; text: string; force?: boolean }) => JournalReferenceResult }) {
  ...
  if (query.isPending) return <section className="quick-notes-route"><div className="notes-skeleton" aria-label="随手记加载中" /></section>;
  ...
  return <section className="quick-notes-route">
    ...
    <form className="quick-note-detail" onSubmit={save}>
      ...
      <div className="quick-note-detail-actions">
        ...
        {!note.deletedAt ? <Button variant="primary" type="submit" loading={pending} disabled={!dirty || !draft.content.trim()}>保存修改</Button> : null}
      </div>
    </form>
...
function ConvertNoteDialog({ note, selectedDate, recordTimezone, categories, request, onClose, onConverted }: { note: QuickNote; selectedDate: string; recordTimezone: string; categories: CategoryOption[]; request: Request; onClose: () => void; onConverted: () => void | Promise<void> }) {
  ...
  async function submit(accept: boolean) {
    ...
    try {
      ...
      await onConverted();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "转换失败"); setPending(null); }
  }

  return createPortal(<div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal-shell" onMouseDown={(event) => event.stopPropagation()}><section className="quick-note-editor note-convert-dialog" role="dialog" aria-modal="true" aria-labelledby="convert-note-title"><header className="quick-note-editor-head"><div><p className="route-eyebrow">确认行动快照</p><h2 id="convert-note-title">转为悬赏</h2></div><Button iconOnly aria-label="关闭" onClick={onClose}><X size={16} /></Button></header><p className="note-length-hint">原文 {note.content.length.toLocaleString()} 字 · 任务描述上限 2,000 字。原随手记不会被改写。</p><label>标题<input className="field" maxLength={200} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>行动摘要<textarea aria-label="任务行动摘要" className="quick-note-body note-action-summary" maxLength={2000} value={description} onChange={(event) => setDescription(event.target.value)} /></label>{tooLong && !description.trim() ? <p className="quick-note-error">原文超过任务上限，请先写下明确的行动摘要。</p> : null}<div className="note-convert-fields"><label>分类<select className="field" value={categoryId} onChange={(event) => setCategoryId(event.target.value)}><option value="">Inbox / 未分类</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label>难度<select className="field" value={difficulty} onChange={(event) => setDifficulty(event.target.value)}><option value="1">轻松</option><option value="2">普通</option><option value="3">困难</option><option value="4">硬仗</option></select></label><label>预计分钟<input className="field" min="1" type="number" value={estimatedMinutes} onChange={(event) => setEstimatedMinutes(event.target.value)} /></label><label>截止时间<input className="field" type="datetime-local" value={dueAt} onChange={(event) => setDueAt(event.target.value)} /></label></div>{error ? <p className="quick-note-error" role="alert">{error}</p> : null}<footer className="note-dialog-actions"><Button onClick={onClose}>取消</Button><Button loading={pending === "publish"} disabled={!title.trim() || !description.trim() || description.length > 2000} onClick={() => void submit(false)}>发布悬赏</Button><Button variant="primary" loading={pending === "accept"} disabled={!title.trim() || !description.trim() || description.length > 2000} onClick={() => void submit(true)}>发布并接取</Button></footer></section></div></div>, document.body);
}
...
function QuoteNoteDialog({ note, selectedDate, onClose, onQuote }: { note: QuickNote; selectedDate: string; onClose: () => void; onQuote: (input: { noteId: number; targetDate: string; text: string; force?: boolean }) => JournalReferenceResult }) {
  ...
  return createPortal(<div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal-shell" onMouseDown={(event) => event.stopPropagation()}><section className="quick-note-editor note-quote-dialog" role="dialog" aria-modal="true" aria-labelledby="quote-note-title"><header className="quick-note-editor-head"><div><p className="route-eyebrow">本设备草稿</p><h2 id="quote-note-title">引用到日记</h2></div><Button iconOnly aria-label="关闭" onClick={onClose}><X size={16} /></Button></header><label>日记日期<input aria-label="日记日期" className="field" type="date" value={targetDate} onChange={(event) => { setTargetDate(event.target.value); setDuplicate(false); }} /></label><label>引用文字<textarea aria-label="引用文字" className="quick-note-body note-action-summary" value={text} onChange={(event) => setText(event.target.value)} /></label>{duplicate ? <p className="quick-note-error">这条随手记已经引用到该日期的草稿或日记。</p> : null}<footer className="note-dialog-actions"><Button onClick={onClose}>取消</Button>{duplicate ? <Button variant="danger" disabled={!text.trim()} onClick={() => quote(true)}>再次引用</Button> : <Button variant="primary" disabled={!text.trim()} onClick={() => quote(false)}>加入草稿并打开日记</Button>}</footer></section></div></div>, document.body);
}
...
function NoteState({ note }: { note: QuickNote }) {
  if (note.deletedAt) return <Badge tone="danger">回收中</Badge>;
  if (note.archivedAt) return <Badge tone="warning">已归档</Badge>;
  return <Badge tone="success">活跃</Badge>;
}
