        ...
        <div className="task-pool-list">
          {filtered.length ? filtered.map((task) => {
            ...
            return <div className={`task-pool-option ${selected ? "task-pool-option-selected" : ""}`} key={task.id}>
              <label data-guide-anchor="task.accept" data-guide-task-id={task.id} className="task-pool-main"><input type="checkbox" checked={selected} onChange={() => toggleTask(task.id, selected)} /><span className="min-w-0 flex-1"><span className="task-pool-line"><strong>{task.title}</strong>{category ? <CategoryTag category={category} /> : <span className="task-pool-empty-category">未分类</span>}<span className="task-pool-inline-meta">发布 {formatDateTime(task.createdAt)}</span><span className="task-pool-inline-meta">{task.dueAt ? `截止 ${formatDateTime(task.dueAt)}` : "暂无截止"}</span><span className="task-pool-inline-meta">{difficultyLabel(task.difficulty)} · {task.estimatedMinutes ? `${task.estimatedMinutes}m` : "未估时"}</span></span></span></label>
