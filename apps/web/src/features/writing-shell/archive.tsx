...
export function WritingArchivePage({ request, userId, selectedDate }: { request: Request; userId: number; selectedDate: string }) {
  ...
  return <section className="writing-archive-route"><header className="writing-archive-head"><div><p className="route-eyebrow">{tx("文字 / Writing")}</p><h1>{tx("归档")}</h1><p>{tx("已从活跃 Writing 内容中移出的记录，可从原业务入口继续恢复。")}</p></div><Archive size={20} aria-hidden="true" /></header>{loading ? <p className="route-state">{tx("正在加载归档...")}</p> : items.length ? <div className="writing-archive-list">{items.map((item) => <article className="writing-archive-item" key={`${item.kind}-${item.id}`}><div className="writing-archive-icon">{item.kind === "note" ? <NotebookPen size={16} /> : <Lightbulb size={16} />}</div><div className="writing-archive-content"><div><span className="writing-archive-kind">{item.kind === "note" ? tx("随手记") : tx("灵感库")}</span><time dateTime={item.date}>{item.date}</time></div><h2>{item.title}</h2><p>{item.excerpt}</p>{item.tag ? <small>#{item.tag}</small> : null}</div><Link className="inline-command" to={`${item.href}${item.href.includes("?") ? "&" : "?"}from=archive`}>{tx("查看原文")}</Link></article>)}</div> : <div className="writing-archive-empty"><Archive size={24} /><h2>{tx("还没有归档内容")}</h2><p>{tx("在随手记或灵感库中归档的内容会出现在这里。")}</p><Link className="inline-command" to={`/writing?date=${selectedDate}`}>{tx("返回文字")}</Link></div>}</section>;
}
