...
function AppShell({ context }: { context: WorkspaceContext }) {
  ...
  useEffect(() => { document.title = `${title} · ${APP_BRAND.name}`; mainRef.current?.focus({ preventScroll: true }); }, [location.pathname, title]);
  ...
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    <aside className="app-sidebar">
      <Link className="app-brand" to={link("/today")}><span aria-hidden="true">{APP_BRAND.icon}</span><strong>{APP_BRAND.name}</strong></Link>
      ...
      <nav aria-label="主导航">
        ...
        <ShellLink to={link("/writing")} activePaths={["/writing", "/journal", "/notes", "/inspirations"]} icon={<NotebookPen size={17} />}>文字</ShellLink>
    ...
    <div className="app-stage">
      ...
      <div className={`quick-add-wrap ${location.pathname.startsWith("/settings") ? "is-mobile-hidden" : ""}`} ref={quickMenuRef}>
        <button data-guide-anchor="quick-action.task.publish.entry" type="button" className="quick-action" aria-controls="quick-add-menu" aria-expanded={quickOpen} aria-label="打开快捷操作" onClick={() => { setQuickOpen((value) => !value); setQuickMoreOpen(false); }}><Plus size={16} /><span>Quick Action</span></button>
        {quickOpen ? <div className={`quick-add-menu ${quickMoreOpen ? "is-more-open" : ""}`} id="quick-add-menu" role="menu">
          ...
          <div className="quick-add-primary">
            <button data-guide-anchor="quick-action.task.publish" onClick={() => { closeQuickMenu(); taskSurfaces.openCreate(); }}><CheckSquare2 size={17} /><span>发布悬赏</span></button>
...
function JournalRoute() {
  ...
  return <WritingShell userId={value.session.user.id} date={value.date} activeId={activeId} enabledSlots={enabledSlots}>
    <section className="journal-route">
      <header className="journal-route-head"><div><p className="route-eyebrow">文字 / Writing</p><h1>{activeId === "morning" ? "晨写" : activeId === "review" ? "复盘" : "日记"}</h1><p>内容按所选日期独立保存。</p></div></header>
...
function MonthCalendar({ from, items, loading, date }: { from: string; items: Schedule[]; loading: boolean; date: string }) { const days = Array.from({ length: 42 }, (_, index) => { const value = new Date(`${from}T00:00:00Z`); value.setUTCDate(value.getUTCDate() + index); return value.toISOString().slice(0, 10); }); return <div className="calendar-month-view"><div className="calendar-month-note">每日实际投入时长</div><div className="calendar-month-grid" role="grid" aria-label="月视图"><div className="calendar-month-weekdays">{["一", "二", "三", "四", "五", "六", "日"].map((day) => <span key={day}>周{day}</span>)}</div>{days.map((day) => { const dayItems = items.filter((item) => item.scheduleDate === day); const actualMinutes = dayItems.filter((item) => item.kind === 1).reduce((sum, item) => sum + scheduleMinutes(item), 0); const level = Math.min(4, actualMinutes ? Math.max(1, Math.ceil(actualMinutes / 60)) : 0); return <Link role="gridcell" aria-label={`${day}，实际投入 ${actualMinutes} 分钟`} className={`${day === date ? "is-selected" : ""} ${day.slice(0, 7) !== date.slice(0, 7) ? "is-outside" : ""}`} key={day} to={`/calendar?date=${day}`}><strong>{Number(day.slice(-2))}</strong>{loading ? <small>...</small> : <span className={`calendar-month-markers heat-level-${level}`} title={`实际投入 ${actualMinutes} 分钟`}>{actualMinutes ? <small>{actualMinutes}m</small> : null}</span>}</Link>; })}</div></div>; }
function YearHeatmap({ from, to, items, loading, date }: { from: string; to: string; items: Schedule[]; loading: boolean; date: string }) {
  ...
  return <section className="calendar-year-view" aria-label="年度实际投入热力图" data-range={from + ":" + to}><div className="calendar-year-heading"><div><strong>{from.slice(0, 4)} 年投入热力图</strong><small>每日实际投入时长</small></div><span>{loading ? "加载中…" : "全年 " + totalMinutes + " 分钟"}</span></div><div className="calendar-year-scroll"><div className="calendar-year-grid" role="grid" aria-label="年度热力图">{Array.from({ length: leading }, (_, index) => <span aria-hidden="true" className="calendar-year-spacer" key={"spacer-" + index} />)}{days.map((day) => { const actualMinutes = totals.get(day) ?? 0; const level = Math.min(4, actualMinutes ? Math.max(1, Math.ceil(actualMinutes / 60)) : 0); return <Link role="gridcell" aria-label={day + "，实际投入 " + actualMinutes + " 分钟"} className={"calendar-year-cell heat-level-" + level + (day === date ? " is-selected" : "")} key={day} title={day + " · " + actualMinutes + " 分钟"} to={"/calendar?date=" + day}><span className="sr-only">{day + "，实际投入 " + actualMinutes + " 分钟"}</span></Link>; })}</div></div><div className="calendar-heat-legend"><span>少</span>{[0, 1, 2, 3, 4].map((level) => <i className={"heat-level-" + level} key={level} />)}<span>多</span></div></section>;
}
